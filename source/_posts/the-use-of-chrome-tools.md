...
![复制http请求的curl](https://img.91temaichang.com/blog/http_request_copy_curl.png)

针对XHR请求，一般业务场景下返回的数据报文都是json数据，因此还可以右键弹出菜单中选择复制response

![复制http返回的response](https://img.91temaichang.com/blog/http_copy_response.png)

#### 12、不一样的Console
在浏览器控制台中打印原始的字符串内容，并且根据不同的"等级"，而文字的颜色有所不同。

![不同等级、不同颜色的console](https://img.91temaichang.com/blog/different_console_info.png)

#### 13、表格形式展示JSON数据
`console.table()`更加直观展示`JSON`格式数据

![表格展示JSON数据](https://img.91temaichang.com/blog/json_table_showing.png)

#### 14、截图
网页截图，一般情况下，我们都是使用第三方工具，其实Chrome已经提供了截图功能，其使用方法如下：

![输入命令使用截图](https://img.91temaichang.com/blog/chrome_capture.png)
...
![web performance](https://img.91temaichang.com/blog/web_performance.png)

+ 截图
+ 内存占用，如果要更加专注的了解内存的情况，可在Memory Tab下录制查看，其右侧的"垃圾桶"按钮是主动进行垃圾回收

![垃圾回收](https://img.91temaichang.com/blog/cabier_recycle.png)

+ js调用栈，可定位到对应的js文件在source中预览

![js调用栈](https://img.91temaichang.com/blog/js_envoke_stack.png)
...
![Call Stack](https://img.91temaichang.com/blog/call_stack.png)
##### 16.3 Scope
...
![dom breakpoints](https://img.91temaichang.com/blog/dom_breakpoints.png)
...
![global breakpoints](https://img.91temaichang.com/blog/global_breakpoints.png)

##### 16.7 Event Listener Breakpoints
事件监听断点，打开这个列表，在触发该事件时进入断点，调试器会停留在触发事件代码行。展示事件列表，选择要监听的事件，勾选即可

![event listener breakpoints](https://img.91temaichang.com/blog/event_listener_breakpoints.png)

#### 17、左侧导航栏
##### 17.1 Page
整个`Source Tab`，顾名思义就是源码面板，默认选择"Page"，在这里可以看到以域名为列表的静态资源文件目录树，点击文件在主视图可以预览静态文件

![page](https://img.91temaichang.com/blog/source_page.png)

##### 17.2 Snippets
代码片段，开发者自己写一段调试代码的代码片段，比如解析`json`数据等工具函数，在代码片段中的代码可以访问当前叶脉呢中的变量和函数，不会因刷新而丢失。
+ 右键即可`run`
+ 不需要的时候可以右键`remove`
![Snippets](https://img.91temaichang.com/blog/snipptes.png)

#### 18、主视图区域
即预览静态资源的区域，类似于编辑器，顶部可以切换已打开的脚本/资源文件。预览是最基本的功能，主视图/面板还有其他重要功能。断点的类型有三种：

![main breakpoints](https://img.91temaichang.com/blog/main_breakpoints.png)
...
![条件断点](https://img.91temaichang.com/blog/condition_breakpoints.png)
...
![黑盒script](https://img.91temaichang.com/blog/black_boax.png)

#### 19 Coverage
代码使用率统计，支持运行时录制统计，在例如我们引入了`jquery`但是只用了一个`hide()`方法，那么对于`jquery`的代码使用率是非常低的

![统计js代码使用率](https://img.91temaichang.com/blog/coverage-js.png)
...
![Rendering](https://img.91temaichang.com/blog/rendering.png)

##### 20.2 Layers
当一个页面足够复杂的时候，在`css`的加持下，可能会出现一些渲染性能问题，这时候，可以通过`Layers`工具显式地查看`DOM`层关系。
在`more tools`菜单中选择`Layers`工具

![Layers](https://img.91temaichang.com/blog/layers.png)
...
![chrome浏览器调试android设备](https://img.91temaichang.com/blog/chrome_debug_android.png)

3. 支持设备、网页`App`、页面、`Chrome`插件、`Works`和其他类型的调试
4. 点击`inspect`就可以如正常调试网页一般来调试移动端里面的`webview`中运行的`web`程序

![chrome inspect 页面内容](https://img.91temaichang.com/blog/chrome_inspect_android.png)
