...
cover: mongodb中的query查询以及聚合管道查询封面.png
---
...
### 查询之后的操作
> :confused: 关于这个db查询以及管道聚合操作之后，返回的什么呢？
> :point_right: 两者函数执行成功后，都返回的一**cursor**对象，那么该对象可以用来做什么？也就是该对象都有哪些操作？方便我们对结果数据进行进一步处理！
:point_down: 整理了关于`cursor`的相关操作，如下图所示：
![mongodb中的cursor方法一览](mongodb中的cursor方法一览.png)

:point_up_2: 这里所列举的是在`mongosh`中针对游标直接调用的相关API，而在实际的项目中，则由各个开发语言驱动来提供对应的API来对游标进行访问！！