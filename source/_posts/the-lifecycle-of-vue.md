...
👆 这里🈶️一个关键地方需要注意的是：
> 从父元素`inject`过来的数据，是非响应式的，如果要采用响应式的方式来获取的话，就需要将这个`provide`所提供的对象给响应式，由于我们的data函数所返回的对象才是响应式的，因此我们需要手动的
> 将自己所关心的对象给`响应式`，这里可以直接借助于**Vue.observer**静态方法，将一个对象转换为响应式的对象即可
